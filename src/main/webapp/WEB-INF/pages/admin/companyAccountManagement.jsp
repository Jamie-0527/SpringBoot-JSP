...
<div style="width: 85%;float:right;">
    <div class="border-bottom" style="background-color: rgb(248,249,250);font-size: 16px;line-height: 50px">
        <span class="font-weight-bold" style="margin: 10px 50px 0px 20px">
            <i class="fa fa-file-text" aria-hidden="true"></i>&nbsp;企业账户管理
        </span>
    </div>
    <%--打印账户信息--%>
        ...
        <ul class="pagination justify-content-center">
            <c:choose>
                <c:when test="${pageNum > 1}">
                    <li class="page-item">
                        <a class="page-link" href="companyAccountManagement?page=${pageNum-1 }" tabindex="-1">Previous</a>
                    </li>
                </c:when>
                <c:otherwise>
                ...
                <c:choose>
                    <c:when test="${i eq pageNum}">
                        <li class="page-item active" aria-current="page">
                            <a class="page-link" href="companyAccountManagement?page=${i}">${i}<span class="sr-only">(current)</span></a>
                        </li>
                    </c:when>
                    <c:otherwise>
                        <li class="page-item"><a class="page-link" href="companyAccountManagement?page=${i}">${i}</a></li>
                    </c:otherwise>
                </c:choose>
            </c:forEach>
            ...
            <c:choose>
                <c:when test="${pageNum < pageCount}">
                    <li class="page-item">
                        <a class="page-link" href="companyAccountManagement?page=${pageNum+1 }" tabindex="-1">Next</a>
                    </li>
                </c:when>
                <c:otherwise>
                    <li class="page-item disabled">
